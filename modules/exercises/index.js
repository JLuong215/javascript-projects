...
const averages = require('./average.js')//Import functions from averages.js.
  ...
  if (response.toLowerCase()==='y'){
    if (i===0){
      ...
      for (let j = 0; j<astronauts.length; j++){
        let avg = averages.averages.averageForStudent(j, scores);//Call 'averageForStudent' here. Pass in j and scores as arguments.
        console.log(`${astronauts[j]}'s test average = ${avg}%.`);
      }
    } else {
      let walker = randomSelect;//Call 'randomSelect' to pick a spacewalker from the astronauts array.
